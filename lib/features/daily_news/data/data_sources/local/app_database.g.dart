...
class _$ArticleDao extends ArticleDao {
  _$ArticleDao(this.database, this.changeListener)
      : _queryAdapter = QueryAdapter(database),
        _articleModelInsertionAdapter = InsertionAdapter(
            database,
            'article',
            (ArticelModel item) => <String, Object?>{
                  'id': item.id,
                  'author': item.author,
                  'title': item.title,
  ...
  final InsertionAdapter<ArticelModel> _articleModelInsertionAdapter;

  final DeletionAdapter<ArticelModel> _articleModelDeletionAdapter;

  @override
  Future<List<ArticelModel>> getAllArticles() async {
    return _queryAdapter.queryList('SELECT * FROM article',
        mapper: (Map<String, Object?> row) => ArticelModel(
            id: row['id'] as int?,
  ...
  @override
  Future<void> insertArticle(ArticelModel article) async {
    await _articleModelInsertionAdapter.insert(
        article, OnConflictStrategy.abort);
  }

  @override
  Future<void> deleteArticle(ArticelModel articleModel) async {
